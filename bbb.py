...
class User(Base):
    ...
    badges = Column(String, default='["Web3 Novice"]')
...
class LearningPath(Base):
    ...
    learning_style = Column(String, default="")  # Store the style
...
# Helper Functions
async def generate_course_outline(knowledge_level: str, path_subject: str, weekly_hours: str = "5", learning_style: str = "article"):
    try:
        # Parse weekly_hours
        if "-" in weekly_hours:
...
async def generate_detailed_content(topic: str, content_type: str = "article"):
    prompt = f"""
...
@app.post("/api/award_points")
async def award_points(data: AwardPointsRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        points = data.points
        activity_type = data.type

        # Define XP points for different activities
        xp_awards = {
            "time": points,  # Points passed directly (e.g., 10 for 5 minutes)
            "path_start": 50,
            "course": 200,
            "test": 150,
            "exam": 300
        ...
        xp_to_add = xp_awards.get(activity_type, points)  # Fallback to passed points if type is unknown
        user.xp_points += xp_to_add

        # Update time spent (for time-based activities or course completion)
        if activity_type == "time":
            user.time_spent += 1  # 1 minute increments
        elif activity_type == "course":
            user.time_spent += 30  # Assume 30 minutes per course
...
async def learning_page(request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        logger.error(f"Error in learning_page: {str(e)}")
        return HTMLResponse(content=f"Error: {str(e)}", status_code=500)
...
async def generate_roadmap(career_id: int, user_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        course_outline = await generate_course_outline("expert", career_name)

        learning_path = LearningPath(
            user_id=user_id, 
            name=f"{career_name} Carrer Path", 
            course_outline=json.dumps(course_outline)
        )
        db.add(learning_path)
        db.commit()
...
async def start_learning_path(level: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        level_name = level['level'].lower()
        path_name = f"{level_name.capitalize()} Path"
        
        # Check if path already exists for the user
        ...
        if existing_path:
            return {"path_id": existing_path.id}

        # Create new path if none exists
        course_outline = await generate_course_outline(level_name, "Web3", "5", "article")
        learning_path = LearningPath(
            user_id=user.id,
            name=path_name,
            course_outline=json.dumps(course_outline)
...
@app.get("/api/user_learning_paths")
async def get_user_learning_paths(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        paths = db.query(LearningPath).filter(LearningPath.user_id == user.id).all()
        return [{"level": path.name.replace(" Path", "").lower(), "path_id": path.id} for path in paths]
    except Exception as e:
...
async def course_page(request: Request, learning_path_id: int, module_index: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        topics = json.loads(learning_path.course_outline) if learning_path.course_outline else []
        if module_index < 0 or module_index >= len(topics):
            logger.error(f"Module index out of range: module_index={module_index}, total_topics={len(topics)}, path_id={learning_path_id}")
            return JSONResponse({"error": f"Module index {module_index} out of range (0-{len(topics)-1})"}, status_code=404)
        ...
        if topic_progress:
            ...
            topic_data.update({
                ...
                "completion_date": topic_progress.completion_date if topic_progress.completion_date else None,
        ...
        if not detailed_content:
            ...
            if topic_progress:
                ...
                db.commit()
            elif learning_path.learning_style != "video":
        ...
        return templates.TemplateResponse("course.html", {
            ...
            "user": user,
            "total_modules": len(topics)
        })
    except Exception as e:
        logger.error(f"Error in course_page: {str(e)}")
        raise  # Re-raise to ensure FastAPI handles the error
...
async def update_topic_progress(data: TopicFeedback, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        if topic.completed and not topic.completed:
            ...
            # Award points
            points_result = await award_points(topic.topic_id, user, db)
...
async def badges_page(request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        # Quiz badges (from TopicProgress)
        quiz_badges = []
        exam_rewards = []
        learning_paths = db.query(LearningPath).filter(LearningPath.user_id == user.id).all()
        for lp in learning_paths:
            topics = db.query(TopicProgress).filter(
                TopicProgress.learning_path_id == lp.id,
                TopicProgress.badge_png_path.isnot(None)
            ).all()
            for topic in topics:
                quiz_badges.append({
                    "topic_id": topic.id,
                    ...
                    "badge_png_path": f"static/{topic.badge_png_path}" if topic.badge_png_path else None,
                    "badge_pdf_path": f"static/{topic.badge_pdf_path}" if topic.badge_pdf_path else None
                })
            if lp.exam_completed and lp.exam_badge_png_path:
                exam_rewards.append({
                    "learning_path_id": lp.id,
                    "learning_path_name": lp.name,
                    "exam_badge_png_path": f"static/{lp.exam_badge_png_path}" if lp.exam_badge_png_path else None,
                    "exam_badge_pdf_path": f"static/{lp.exam_badge_pdf_path}" if lp.exam_badge_pdf_path else None,
                    "certificate_pdf_path": f"static/{lp.certificate_pdf_path}" if lp.certificate_pdf_path else None
                })

        # User-level badges (from user.badges)
        user_badges = json.loads(user.badges) if user.badges else []
        user_badge_items = [
            {
                "name": badge,
                "badge_png_path": f"static/badges/{badge.lower().replace(' ', '_')}.png",  # Assumes badge images exist
                "badge_pdf_path": f"static/badges/{badge.lower().replace(' ', '_')}.pdf"   # Assumes badge PDFs exist
...
@app.get("/api/get_theme")
async def get_theme(user: User = Depends(get_current_user)):
    try:
        return {"theme": user.theme_preference or "dark"}
    except Exception as e:
