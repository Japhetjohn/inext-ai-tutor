...
class Web3Manager:
    def __init__(self):
        self.project_id = os.getenv("WALLETCONNECT_PROJECT_ID")
        self.connections: Dict[str, Dict] = {}

    async def create_session(self) -> str:
        """Create a new WalletConnect session."""
        try:
            return f"wc:{os.urandom(16).hex()}@2?projectId={self.project_id}"
    ...
    def get_chain_config(self, chain_id: str) -> Optional[Dict]:
        """Get configuration for supported chains."""
        if chain_id not in SUPPORTED_CHAINS:
            return None

        return {
            "name": SUPPORTED_CHAINS[chain_id],
            "chain_id": chain_id,
            "is_testnet": False
        }

wallet_manager = Web3Manager()
...
async def connect_wallet():
    ...
    try:
        uri = await wallet_manager.create_session()
...
async def send_tokens_endpoint(request: TokenRequest):
    ...
    if not wallet_manager.verify_signature(request.wallet_address, request.message, request.signature):
